---
name: pdf2md
description: PDFファイルをMarkdownに変換するスキル。1ファイルでも複数ファイルでも対応。DocLayout-YOLOによる図版抽出・画像埋め込み付きで高品質なMarkdownを生成し、PDFファイル名のフォルダにまとめて保存する。「PDFをMarkdownにして」「PDFを変換して」「DownloadsにあるXXXをmdに変換して」「○○フォルダ内のPDFを全部Markdownにして」など、PDF→Markdown変換に関する依頼があれば必ずこのスキルを使うこと。他のスキルへの依存なし。
---

# pdf2md

PDFファイルを受け取り、**PDFファイル名（拡張子なし）のフォルダ**を作成して以下のファイルを格納するスキル。1ファイルでも複数ファイルでも動作する。

```
<出力先>/<PDFファイル名>/
├── images/                 — PDFから抽出した図表画像
└── paper.md                — Markdown変換（画像埋め込み）
```
...
1. PDFのファイル名から拡張子（`.pdf`）を除いた名前をフォルダ名として使用する。
   - 例: `attention_is_all_you_need.pdf` → `attention_is_all_you_need/`
2. PDFと同じディレクトリに `<PDFファイル名>/` フォルダを作成する。
3. 以降の全ファイルはこのフォルダ内に保存する。出力パスを `<出力フォルダ>` とする。
